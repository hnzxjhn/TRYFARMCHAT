...
class FarmCHATMessenger
{
    ...
    {
        return config('FarmCHAT.attachments.max_upload_size') * 1048576;
    }

    public function __construct()
    {
        $this->pusher = new Pusher(
            config('FarmCHAT.pusher.key'),
            config('FarmCHAT.pusher.secret'),
            config('FarmCHAT.pusher.app_id'),
            config('FarmCHAT.pusher.options'),
        );
    }
    /**
    ...
    {
        return config('FarmCHAT.attachments.allowed_images');
    }
    ...
    {
        return config('FarmCHAT.attachments.allowed_files');
    }
    ...
    public function messageCard($data, $renderDefaultCard = false)
    {
        ...
        return view('FarmCHAT::layouts.messageCard', $data)->render();
    }
    ...
    public function getContactItem($user)
    {
        try {
            ...
            return view('FarmCHAT::layouts.listItem', [
                'get' => 'users',
    ...
    {
        if ($user->avatar == 'avatar.png' && config('FarmCHAT.gravatar.enabled')) {
            $imageSize = config('FarmCHAT.gravatar.image_size');
            $imageset = config('FarmCHAT.gravatar.imageset');
    ...
    public function deleteConversation($user_id)
    {
        try {
            foreach ($this->fetchMessagesQuery($user_id)->get() as $msg) {
                ...
                if (isset($msg->attachment)) {
                    $path = config('FarmCHAT.attachments.folder').'/'.json_decode($msg->attachment)->new_name;
    ...
    {
        return Storage::disk(config('FarmCHAT.storage_disk_name'));
    }
    ...
    {
        return self::storage()->url(config('FarmCHAT.user_avatar.folder') . '/' . $user_avatar_name);
    }
    ...
    {
        return self::storage()->url(config('FarmCHAT.attachments.folder') . '/' . $attachment_name);
    }
}
